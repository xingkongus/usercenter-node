...
class UserController extends Controller {
  async regist() {
    ...
    if (email) {
      ...
      if (queryUserRes.result) {
        ctx.success(ctx, { msg: '邮箱已被注册', code: 412 });
        return 0;
      }
    } else {
  ...
  async login() {
    ...
    let cryptoPwd = this.ctx.helper.cryptoPwdGenerator(password);
    let queryUserRes = null;
    if (username) {
      queryUserRes = await this.ctx.service.user.getUser({ username: username });
    } else if (email) {
      queryUserRes = await this.ctx.service.user.getUser({ email: email });
    }
    if (queryUserRes.result && queryUserRes.data.password === cryptoPwd) {
      ...
      this.ctx.cookies.set('token', token, { maxAge: 60 * 60 * 1000, httpOnly: false, overwrite: true, signed: false });
      this.ctx.success(ctx, { result: responseRes, msg: '登录成功' });
    } else {
      if (queryUserRes.fatal) {
        ctx.error(ctx, { code: 500, msg: queryUserRes.message });
      } else {
        ctx.body = new ctx.commonResEntity({ data: "", msg: '用户名或密码错误', status: 'error', code: 500 });
      }
    }
  }
